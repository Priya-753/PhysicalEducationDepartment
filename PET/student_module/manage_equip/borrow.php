    ...
    $studid = $_SESSION['userid'];
		...
		if(isset($_POST['submit'])){
			...
			if($username == ''){
				...
				$stmt = $databaseConnection->prepare("SELECT * FROM equipmentDetails WHERE equipID = :equipid and status = 'a'");
				$stmt->bindValue(':equipid', $username);
				$stmt->execute();
				$userRow=$stmt->fetch(PDO::FETCH_ASSOC);
				if($stmt->rowCount() > 0)
				{
						 $stmt1 = $databaseConnection->prepare("UPDATE equipmentDetails SET status = 'ua', borrowed = :studid where equipID = :equipid");
             $stmt1->bindValue(':studid', $studid);
             $stmt1->bindValue(':equipid', $username);
             $stmt1->execute();
             $msg="Equipment Borrowed";
             echo "<span class='res'>" . $msg . "</span>";
				}
		 else{
			 $msg="Equipment with that ID is not available.";
			 echo "<span class='res'>" . $msg . "</span>";
		 }
		}
